...
const Login = () => {
    ...
    const onClickLogin = () => {
        ...
        axios.post('http://52.78.37.198/onLogin', null, { //axios 모듈에서 .post 는 아래와 같이 매개변수 3개이고, params 를 config 로 전달해야 하기 때문에 중간에 data 값을 null 로 넣어 주었다.
            params: {
            'user_id': inputId,
            'user_pw': inputPw
            }
    ...
    useEffect(() => {
        axios.get('http://52.78.37.198/login') 
        // 404 에러해결 : 서버의 포트에서 받아와야 하는데 그냥 /login이라고 하면 클라이언트의 포트(localhost:3000/login)으로 받아온다. 실제론 52.78.37.198/login에서 받아와야 함
